# Releasing `@chaosity/location-client`
...
so the branch tip becomes one of its parents and `git log v0.1.14..v0.2.0` and
`git switch --detach v0.2.0` work as normal.

Do this in its **own PR**, separate from the change it releases: the diff is two
lines and it is unambiguous what merging it does.
...
```bash
npm view @chaosity/location-client versions --json    # compare against:
git tag -l 'v*'
```
...
```bash
npm view @chaosity/location-client version
npm audit signatures
curl -s "https://registry.npmjs.org/-/npm/v1/attestations/$(node -p \
  "encodeURIComponent('@chaosity/location-client')")@<version>" | jq
```
